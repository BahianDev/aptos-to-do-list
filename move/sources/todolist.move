module todolist_addr::todolist {
    ...
    const E_INSUFICIENT_APTOS_COIN: u64 = 4;
    ...
    public entry fun init(sender: &signer) {
        assert!(signer::address_of(sender) == @todolist_addr, ENOT_ROOT);
        let zero = coin::zero<AptosCoin>();
        move_to(sender, Bank<AptosCoin>{
            balance:zero
        })
    }


    public entry fun create_list(account: &signer) acquires Bank{
        let c = coin::withdraw<AptosCoin>(account, PRICE);
        let account_address = signer::address_of(account);
        assert!(coin::balance<AptosCoin>(account_address) >= PRICE, E_INSUFICIENT_APTOS_COIN);
        let bank = borrow_global_mut<Bank<AptosCoin>>(@todolist_addr);
        coin::merge(&mut bank.balance,c);
    ...
    public entry fun delete_task(account: &signer, task_id: u64) acquires TodoList {
        ...
        // assert task exists
        assert!(table::contains(&todo_list.tasks, task_id), ETASK_DOESNT_EXIST);
        // removes the task matched the task_id
